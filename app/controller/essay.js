...
module.exports = {
    ...
    save: (req, res) => {
        upload(req, res, (err) => {
            ...
            // let originalName = poster ? poster.originalname : 'Starry.jpg';
            // let changePath = `public/images/${originalName}`;
            // let path = `/images/${originalName}`;
            let _essay;

            // fs.renameSync(poster ? poster.path : `public/images/${originalName}`, changePath);

            if (id === 'undefined') {
                _essay = new EssayModel({
                    title: essayObj.title,
                    poster: essayObj.poster || 'http://p7u4khtgt.bkt.clouddn.com/login.jpg',
